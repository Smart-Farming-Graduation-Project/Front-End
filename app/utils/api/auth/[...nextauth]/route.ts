...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, account, profile }) {
      ...
      if (account) {
        try {
          ...
          // Handle backend authentication based on provider
          if (account.provider === "google" || account.provider === "facebook") {
            ...
            const refreshToken = response.data.data.tokens.refreshToken; // Get refreshToken
            // Save both tokens to the JWT
            token.backendToken = backendToken;
            token.refreshToken = refreshToken; // Store refreshToken in JWT
    ...
    async session({ session, token }) {
      // Add both tokens to the session
      session.backendToken = token.backendToken as string;
      session.refreshToken = token.refreshToken as string; // Add refreshToken to session
      return session;
    },
  },
  pages: {
