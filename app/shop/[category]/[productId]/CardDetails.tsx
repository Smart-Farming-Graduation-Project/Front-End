...
} from "@/app/utils/redux/slices/wishListSlice";
import {
  addToCartAPI,
  deleteProductAPI,
} from "@/app/utils/redux/slices/CartSlice";
...
const CardDetails = ({ product }: { product: ProductId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Trigger review re-fetch
      const refreshedReviews = await getReviews(
        product.productId,
        token as string
      );
              ...
              <div className="w-24 h-12 border border-gray-300 rounded-lg flex items-center overflow-hidden">
                ...
                <div className="flex flex-col h-full border-l border-gray-300 w-1/3">
                  <button
                    ...
                    className="flex-1 h-1/2 flex items-center justify-center text-lg font-semibold border-b border-gray-300 hover:bg-gray-200 active:bg-gray-300 transition"
                  >
                    <FaPlus size={12} />
                  </button>
                  <button
                    onClick={handleDecrement}
                    className="flex-1 h-1/2 flex items-center justify-center text-lg font-semibold hover:bg-gray-200 active:bg-gray-300 transition"
                  >
                    <TiMinus size={12} />
                  </button>
                </div>
          ...
          <form className="flex flex-col gap-1 mt-2" onSubmit={handleSubmit}>
            <div className="flex items-center gap-1.5 mb-2">
              <Input
                ...
                className="pl-2 py-[20px] text-sm bg-light-yellow-section shadow-none"
                value={headline}
            ...
            <Textarea
              ...
              className="input h-24 text-sm bg-light-yellow-section shadow-none"
              value={reviewText}
              onChange={(e) => setReviewText(e.target.value)}
              required
            />
            <Button className="shadow-none bg-yellow py-6 px-6 hover:bg-[#eec451] w-full sm:w-fit mt-3">
              Add Review
            </Button>
          </form>
        </div>
